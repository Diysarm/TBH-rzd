...
// Prices market_hash_names via priceoverview (currency-controllable). Per-item

// and rate-limited, so refresh is throttled with exponential backoff on HTTP

// 429 and runs until the queue finishes (no early stop). Incremental: names

// priced within the freshness TTL are skipped. Cached per-currency in
...
export class SteamMarketProvider {
  ...
  /** Names with no cache entry or a stale one. */
  ...
  async refresh(
    ...
    try {
      ...
      for (let i = 0; i < allTargets.length; i++) {
        ...
        if (priced > 0 && priced % 5 === 0) this.persist();
